...
package org.apache.jmeter.protocol.oauth.sampler.signature;

import org.apache.jmeter.protocol.oauth.sampler.*;
...
public abstract class OAuthSignatureMethod {
    ...
    throws OAuthException, IOException, URISyntaxException {
        ...
        log.debug(signature + "=getSignature(" + baseString + ")");
        return signature;
    }
    ...
    protected static String normalizeParameters(
        ...
        for (Map.Entry<String, String> parameter : parameters) {
            ...
            if ("".equals(parameter.getKey())){
                ...
                //todo POST parameters should only be included in the signature if they are of content-type "application/x-www-form-urlencoded" as with a form submission.
                // todo make it an option in the ui
//                p.add(new ComparableParameter(body));
            }
            // oauth_signatureの元となる文字列に含めてはいけない文字列を除外します
            // todo include oauth_version into the signature computation otherwise
            // user will get an invalid signature response
            else if (!"oauth_signature".equals(parameter.getKey()) &&
                     /*!"oauth_version".equals(parameter.getKey()) && */
                     !"auth_token".equals(parameter.getKey())) {
